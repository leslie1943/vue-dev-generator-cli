...
function page() {
  ...
  inquirer.prompt(questions).then(function (answer) {
    if(answer.name == ""){
      console.log(chalk.red.bold("page name is required"));
    }else{
      ...
      setTargetPath(function (target) {
        ...
        try {
          fs.writeFileSync(`${cPath}/${name}.vue`, template.page({ name: name, type: 'page' }), 'utf8');
        ...
        //Write scss file.
        try {
          fs.writeFileSync(`${cPath}/main.scss`, template.scss({ name: name, type: 'page' }), 'utf8');
...
function component() {
  ...
  inquirer.prompt(question).then(function (answer) {
    if(answer.name == ""){
      console.log(chalk.red.bold("component name is required"));
    }else{
      const name = answer.name;
      setTargetPath(function (target) {
        const cPath = path.join(target, `./${name}`);
        ...
        //Write vue file.
        try {
          fs.writeFileSync(`${sPath}/${name}.vue`, template.page({ name: name, type: 'component' }), 'utf8');
        ...
        //Write scss file.
        try {
          fs.writeFileSync(`${sPath}/main.scss`, template.scss({ name: name, type: 'component' }), 'utf8');
...
function api() {
  ...
  inquirer.prompt(question).then(function (answer) {
    if(answer.name == ""){
      console.log(chalk.red.bold("api name is required"));
    }else{
      const name = answer.name;
      ...
      inquirer.prompt(q2).then(function (api) {
        if(api.name == ""){
          ...
          inquirer.prompt(q3).then(function (a3) {
            if(a3.name == ""){
              console.log(chalk.red.bold("module name is required"));
            }else{
              setTargetPath(function (target) {
                const name = a3.name;
                // check the diretory,create it if no exists.
